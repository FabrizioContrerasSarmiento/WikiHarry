...
document.getElementById("btn-ver-mas").addEventListener("click", cards15);
    ...
    .then(response => {
        if (!response.ok) {
            ...
            throw new Error("Respuesta no OK. Estado: " + response.status); 
        }
    ...
    function crearCard(article) {
        ...
        boton.addEventListener("click", () => {
            localStorage.setItem("personajeSeleccionado", JSON.stringify(article)); // Guardo en LocalStorage
            const nombreURL = article.name.replace(/\s+/g, ''); //Toma el nombre del personaje y le sacamos todos los espacios con replace
            window.location.href = `personajesInfo.html?personaje=${nombreURL}`; // Redirige al navegador a otra página y le pasamos un parámetro en la URL
        });
